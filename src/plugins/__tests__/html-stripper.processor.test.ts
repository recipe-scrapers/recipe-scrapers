...
describe('HtmlStripperPlugin', () => {
  ...
  it('strips HTML from string values', () => {
    expect(plugin.process('title', '<b>Hello &amp; World</b>')).toBe('Hello & World')
    expect(plugin.process('title', 'No tags')).toBe('No tags')
    expect(plugin.process('title', '<span>Test &lt;tag&gt;</span>')).toBe('Test <tag>')
    expect(plugin.process('description', '<span>Hello&nbsp;World</span>')).toBe('Hello World')
  })
  ...
  it('strips HTML from ingredients', () => {
    ...
    expect(output[0].items.map((i) => i.value)).toEqual(['1 cup flour', '2 <b>eggs</b>'])
  })
  ...
  it('strips HTML from ingredient groups', () => {
    const input: Ingredients = [
      ...
      createIngredientGroup('Other', [createIngredientItem('<span>3</span> apples')]),
    ]
    const output = plugin.process('ingredients', input) as Ingredients
  ...
  it('returns value unchanged for non-target fields', () => {
    expect(plugin.process('category', new Set(['<b>cat</b>']))).toEqual(new Set(['<b>cat</b>']))
  })

  it('decodes all HTML entities', () => {
    expect(plugin.process('title', 'Cr&egrave;me br&ucirc;l&eacute;e')).toBe('Crème brûlée')
    expect(plugin.process('title', '&#x27;quoted&#x27;')).toBe("'quoted'")
    expect(plugin.process('title', '100&deg;C')).toBe('100°C')
    expect(plugin.process('siteName', 'Mamá Maggie&#039;s Kitchen')).toBe("Mamá Maggie's Kitchen")
  })
})
